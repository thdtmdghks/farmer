    ...
    projects: [
      {
        name: '결제 백오피스 & 어드민',
        details: [
          '가맹점 백오피스 + 내부 어드민 프론트엔드 아키텍처 설계 및 초기 구현 주도',
          'MSW mock으로 백엔드 개발 완료 전에도 신규 피처 개발·QA를 병행할 수 있는 구조 확립',
          'Turborepo 모노레포로 2앱 간 공유 코드를 한 번 수정으로 동시 반영되도록 구조화',
        ],
      },
      {
        name: '블록체인 지갑 API',
        details: [
          'NestJS 기반 설계 → 구현 → 테스트 → 문서화 단독 수행',
          'MPC 동시 서명 충돌을 인메모리 락으로 해결하여 연속 전송 시 안정성 확보',
          '외부 API를 nock으로 격리하여 네트워크 상태와 무관한 결정론적 테스트 환경 구축',
        ],
      },
      {
    ...
    projects: [
      {
        ...
        details: [
          ...
          'MSW 전환으로 백엔드 완성 전에도 피처 개발·모바일 QA를 바로 진행할 수 있는 환경 구축',
          'Playwright E2E + CI 배포 차단으로 수동 QA 없이 안정적 배포 가능',
          'pnpm 모노레포 전환으로 공통 코드 수정 시 양쪽 서비스에 즉시 반영',
        ],
      },
      {
        name: '외상 결제 데모몰',
        details: [
          'Next.js SSR로 프론트+서버를 직접 구현, 영업팀 데모 시연에 활용',
          'MSW 도입으로 별도 서버 없이 모바일·에러 케이스 즉시 검증 가능',
        ],
      },
      {
        name: '사업자 정보 검증 서버리스 API',
        details: [
          'BE 리소스 부재 시 자진하여 단독 수행 — 가입 플로우 블로킹 해소',
          '3레이어 분리 + 3단계 테스트로 처음 접하는 기술 스택에서도 안정적 운영',
        ],
      },
      {
        name: '백오피스 리뉴얼',
        details: [
          'Vue → Next.js 전면 리뉴얼',
          'Split Panel UI + 달력 수수료 계산기 제안·구현으로 운영팀 수작업 절감',
        ],
      },
    ],
  },
    ...
    projects: [
      {
        ...
        details: [
          '타임라인·드래그·스냅·멀티 셀렉션 코어 기능 개발',
          'Canvas 렌더링으로 DOM 프레임 하락 해결, 로그 스케일 줌으로 정밀 편집 지원',
        ],
      },
      {
        name: '보도정보시스템 고도화',
        details: [
          'Node.js 버전 불일치 근본 원인 추적 → 3개월 지연 프로젝트를 2주 조기 완료',
        ],
      },
      {
        name: 'CMS 재구축',
        details: [
          '24시간 모니터링 개발 + Heap Snapshot으로 메모리 누수 근본 원인 특정·해결',
        ],
      },
    ],
  },
