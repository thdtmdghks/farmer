export function Resume1Summary() {
  return (
    <>
      {/* 헤더 */}
      ...
      <section className="mb-8">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="rounded-lg border-l-4 border-l-indigo-300 bg-gray-50 py-3 pr-4 pl-4">
            ...
            <ul className="mt-1.5 space-y-1.5 pl-2 text-sm text-gray-600">
      ...
      <section className="mb-8 rounded-lg bg-gray-50 p-6">
        ...
        <div className="mb-6">
          ...
          <p className="pl-1 text-sm text-gray-500 italic">
            백오피스 모노레포 아키텍처 구축 및 블록체인 지갑 도메인 개발 전담
          </p>
          <ul className="mt-2 space-y-3 border-l-2 border-gray-200 pl-4 text-sm text-gray-600">
            <li>
              · Turborepo 모노레포 아키텍처 설계 및 구축
              <p className="mt-1.5 pl-2">
                → ESLint 정적 분석을 통한 단방향 의존성 강제로 아키텍처 무결성
                확보 및 독립 개발 환경 확립
              </p>
            </li>
            <li>
              · NestJS 기반 블록체인 지갑 API 구현
              <p className="mt-1.5 pl-2">
                → 지갑 단위의 비동기 대기열 적용으로 MPC 서명 병목 및 거부 에러
                해결
              </p>
            </li>
            <li>
              · nock 기반 외부 API 모킹 및 DB 격리를 적용한 결정론적 E2E 테스트
              파이프라인 구축
              <p className="mt-1.5 pl-2">
                → 외부 의존성에 흔들리지 않는 신뢰할 수 있는 테스트 환경 확보
              </p>
            </li>
          </ul>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="pl-1 text-sm text-gray-500 italic">
            첫 FE 개발자로 합류, 결제 MVP 구현 후 2인 프론트엔드 파트 리드
          </p>
          <ul className="mt-2 space-y-3 border-l-2 border-gray-200 pl-4 text-sm text-gray-600">
            <li>
              · 간편결제 MVP 구현 및 Playwright E2E 자동 테스트 구축
              <p className="mt-1.5 pl-2">
                → 결제 시나리오 자동 검증 및 결함 코드의 배포 차단
              </p>
            </li>
            <li>
              · JSON Server 환경을 MSW 네트워크 모킹으로 마이그레이션 주도
              <p className="mt-1.5 pl-2">
                → 모바일 실기기 및 테스트 환경 QA 생산성 개선
              </p>
            </li>
            <li>
              · AWS SAM 기반 서버리스 API 구축 및 LocalStack 연동
              <p className="mt-1.5 pl-2">
                → 통합 테스트 환경 구축을 통한 무장애 운영
              </p>
            </li>
            <li>
              · 정산 어드민 Next.js 리뉴얼 및 복잡 연산 유닛 테스트 구축
              <p className="mt-1.5 pl-2">
                → 분할 패널 UI 도입으로 정산 운영 동선 개선 및 연산 무결성 확보
              </p>
            </li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="mt-2 space-y-3 border-l-2 border-gray-200 pl-4 text-sm text-gray-600">
            <li>
              · 웹 영상 편집기 타임라인 드래그 구현
              <p className="mt-1.5 pl-2">
                → Canvas API 하이브리드 렌더링으로 브라우저 프레임 하락 극복
              </p>
            </li>
            <li>
              · 배포 서버 간 Node.js 빌드 엔진 버전 표준화
              <p className="mt-1.5 pl-2">
                → 라이브러리 빌드 버그 해결로 지연되던 프로젝트 정상화 및 2주
                조기 배포
              </p>
            </li>
            <li>
              · Chrome DevTools Heap Snapshot 메모리 프로파일링
              <p className="mt-1.5 pl-2">
                → 미해제 타이머 및 DOM 참조 누수 해결로 무중단 가동 확보
              </p>
            </li>
          </ul>
        </div>
      ...
      <section className="mb-8">
        ...
        <div className="mb-4 rounded-lg border border-gray-200 p-5">
          ...
          <ul className="mt-2 space-y-3 pl-4 text-sm text-gray-600">
            <li>
              · 실제 업체용 홈페이지 설계·개발·배포 (시공 사례, 리뷰, 어드민
              CMS)
            </li>
            <li>
              · Repository 패턴 인터페이스 도입
              <p className="mt-1.5 pl-2">
                → Supabase 네트워크 장애 시에도 정상 빌드/구동되는 독립 개발
                환경 확보
              </p>
            </li>
            <li>
              · Gemini API 이미지 분석 기반 설명글 및 제목 자동 생성
              <p className="mt-1.5 pl-2">
                → 페이로드 한도 초과 시 AI 분석 스킵 및 Fallback 우회로 API 장애
                예방
              </p>
            </li>
            <li>
              · 실시간 캐시 무효화 전략 적용
              <p className="mt-1.5 pl-2">
                → 정적 페이지의 SEO 성능 유지와 실시간 데이터 정합성 양립
              </p>
            </li>
            <li>
              · JSON-LD 구조화 데이터 및 동적 sitemap 적용
              <p className="mt-1.5 pl-2">
                → 구글 라이트하우스 SEO 점수 최적화 및 검색엔진 노출 강화
              </p>
            </li>
          </ul>
        </div>
        ...
        <div className="rounded-lg border border-gray-200 p-5">
          ...
          <ul className="mt-2 space-y-3 pl-4 text-sm text-gray-600">
            <li>
              · ESLint Flat Config 정적 분석 규칙 설계
              <p className="mt-1.5 pl-2">→ 레이어 간 단방향 의존성 자동 강제</p>
            </li>
            <li>
              · Git 커밋 단계 검증 자동화
              <p className="mt-1.5 pl-2">→ 아키텍처 규칙 위반 커밋 차단</p>
            </li>
            <li>
              · Intersection Observer API 및 CSS GPU 가속 활용
              <p className="mt-1.5 pl-2">
                → 라이브러리 의존성 없는 경량 스크롤 애니메이션 구현
              </p>
            </li>
          </ul>
        </div>
