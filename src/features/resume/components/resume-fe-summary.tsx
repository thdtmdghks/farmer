...
const feCareerSummary = [
  {
    ...
    bullets: [
      'React + TypeScript 기반 어드민 2개 앱 설계·구현 (TanStack Router/Query, Zustand)',
      '공유 디자인 시스템 패키지 구축 — shadcn/ui 기반 컴포넌트 라이브러리, 웹 접근성 준수',
      'React 컴포넌트를 독립적으로 개발·검증할 수 있는 MSW 기반 Mock 환경 설계',
      'React Hook Form + Zod 기반 폼 검증 구조 설계, 런타임 에러 사전 차단',
      'Vitest + RTL 유닛 테스트 적용 — 컴포넌트 리팩토링 시 회귀 버그 방지',
      '모노레포(Turborepo) 7개 패키지 구조로 공통 UI의 재사용성·일관성 확보',
    ],
  },
  {
    company: '(주)파이노버스랩',
